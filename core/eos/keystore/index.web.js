...
*/
export default function Keystore(accountName, config = {}) {
  ...
  const uriRulesAndActions = UriRules(config.uriRules)
  ...
  */
  function addKey(path, key, toDisk = false) {
    ...
    if (uriRulesAndActions.deny(currentUriPath(), path).length) {
      // console.log('Keystore addKey denied: ', currentUriPath(), path);
      return null
    }

    const wif = keyType === 'wif' ? key : keyType === 'privateKey' ? ecc.PrivateKey(key).toWif() : null

    const pubkey = keyType === 'pubkey' ? ecc.PublicKey(key).toString() : keyType === 'privateKey' ? key.toPublic().toString() : ecc.privateToPublic(wif)
    ...
    function query(store) {
      userStorage.query(store, [accountName, 'kpath'], ([path, pubkey], wif) => {
        ...
        if (minimatch(path, keyPathMatcher)) {
          ...
          result.deny = uriRulesAndActions.deny(currentUriPath(), path).length !== 0
    ...
    if (deriveKeys.length) {
      for (const derivedKey of deriveKeys) { // {path, privateKey}
        if (derivedKey.path === path) { // filter intermediate children
          const deny = uriRulesAndActions.deny(currentUriPath(), path).length !== 0
          key = {
            path,
  ...
  */
  function timeUntilExpire() {
    return (
      expireAt === 0 ? 0 : expireAt == null ? 0 : Math.max(0, expireAt - Date.now())
    )
  }
  ...
  function deriveKeys({
    ...
    if (!unlistenHistory) {
      unlistenHistory = globalConfig.history.listen(() => {
        ...
        const pathsToPurge = uriRulesAndActions.check(currentUriPath(), paths).deny
