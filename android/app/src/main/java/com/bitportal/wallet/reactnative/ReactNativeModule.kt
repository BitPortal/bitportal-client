...
class ReactNativeModule private constructor(val reactNativeHost: ReactNativeHost) {
  companion object {
    private lateinit var instance: ReactNativeModule
    private val initialized = AtomicBoolean()

    @JvmStatic
    val shared: ReactNativeModule get() = instance

    @JvmStatic
    fun initialize(application: Application, rnHost: ReactNativeHost) {
      if(!initialized.getAndSet(true)) {
        instance = ReactNativeModule(rnHost)
