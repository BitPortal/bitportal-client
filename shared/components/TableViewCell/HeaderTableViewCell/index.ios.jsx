  ...
  return (
    <View style={{ flex: 1, justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', paddingLeft: 15, opacity: props.data.switching ? 0.4 : 1 }}>
      <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
        ...
        {props.data.loading && <Text style={{ fontSize: 20, color: props.data.isDarkMode ? 'white' : 'black' }}>{props.data.loadingTitle}</Text>}
        {!props.data.loading && <Text style={{ fontSize: 20, color: props.data.isDarkMode ? 'white' : 'black' }}>{props.data.title}</Text>}
      </View>
