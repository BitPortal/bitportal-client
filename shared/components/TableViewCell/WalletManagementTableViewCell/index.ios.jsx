    ...
    return (
      <View style={{ flex: 1, justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', paddingLeft: 16, paddingRight: 2 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
          ...
          />
          <Text style={{ fontSize: 17, color: props.data.actionType === 'delete' ? '#FF2D55' : (props.data.isDarkMode ? 'white' : 'black') }}>{props.data.text}</Text>
        </View>
        <View>
      ...
      <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <View style={{ flex: 1, height: 44, borderWidth: 0, borderColor: 'red', justifyContent: 'space-between' }}>
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
            <Text style={{ fontSize: 17, color: 'black', marginRight: 5, fontWeight: '500', color: props.data.isDarkMode ? 'white' : 'black' }}>{props.data.name}</Text>
          </View>
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
            {props.data.address && <Text style={{ fontSize: 15, color: props.data.isDarkMode ? 'white' : 'black' }}>{this.formatAddress(props.data.address)}</Text>}
            {props.data.account && <Text style={{ fontSize: 15, color: props.data.isDarkMode ? 'white' : 'black' }}>{props.data.account}</Text>}
          </View>
        </View>
      </View>
