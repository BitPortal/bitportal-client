...
export default class TradeRAMForm extends Component {
    ...
    this.setState({ isVisible: false }, () => {
      InteractionManager.runAfterInteractions(() => {
        ...
        if (this.state.activeForm === "Buy") {
          this.props.actions.buyRAMRequested(
            data
          ...
          this.props.actions.sellRAMRequested(
            data
              .set("eosAccountName", eosAccountName)
              .set("password", password)
              .toJS()
  ...
  render() {
    ...
    const availableRAM =
      eosAccount.getIn(["data", "ram_quota"]) && eosAccount.getIn(["data", "ram_usage"])
        ? eosAccount.getIn(["data", "ram_quota"]) - eosAccount.getIn(["data", "ram_usage"])
        : 0;
    ...
    return (
      <IntlProvider messages={messages[locale]}>
        <View style={styles.buyRAMForm}>
          ...
          />
          <Balance
            title={messages[locale].resource_label_balance}
            value={availableBalance}
            unit={availableBalanceUnit}
          />
          <FormContainer>
            ...
            <SubmitButton
              ...
              text={
                this.state.activeForm === "Buy"
                  ? messages[locale].resource_ram_button_buy
                  : messages[locale].resource_ram_button_sell
              }
            />
            <Alert
              message={errorMessages(error, messages[locale])}
              subMessage={errorMessageDetail(error)}
              dismiss={this.props.actions.clearRAMError}
            />
            <Alert
