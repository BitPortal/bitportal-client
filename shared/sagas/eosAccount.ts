...
import { initAccount, generateMasterKeys, deriveKeys, getEOS, getLocalAccounts } from 'eos'
import { getMasterSeed, getEOSKeys } from 'key'
...
function* createAccountRequested(action: Action<CreateAccountParams>) {
  ...
  try {
    ...
    const masterWallet = await getMasterSeed()
    console.log(masterWallet)
    const eosWallet = await getEOSKeys(masterWallet.seed)
    console.log(eosWallet)
