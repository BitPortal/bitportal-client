...
export default handleActions(
  {
    ...
    [actions.getChartSucceeded](state, action) {
      return state
        .set('loaded', true)
        .set('loading', false)
        .setIn(['data', action.payload.chartType], Immutable.fromJS(action.payload.data))
    },
    ...
    [actions.changeChartRange](state, action) {
      return state
        ...
        .set('chartType', Immutable.fromJS(RANGE_TO_CHART_TYPE[action.payload]))
    },
    [actions.clearChart](state, action) {
      return state.set('data', Immutable.fromJS({ histominute: [], histohour: [], histoday: [] }))
    }
  },
  initialState
)
